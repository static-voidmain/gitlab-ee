...
```bash
INTERNAL_IMAGE_REGISTRY=registry.example.co.kr/scm-runners ./images/build-images.sh --include-shell-tools
INTERNAL_IMAGE_REGISTRY=registry.example.co.kr/scm-runners ./images/build-images.sh --include-legacy
...
- Maven과 Gradle은 내부 JDK runner 레이어를 재사용하여 registry 저장 공간과 전송량을 줄입니다.
...
- `node:24`, `npm:24`, `pnpm:24`는 동일 이미지에 붙는 별칭이므로 registry blob을 중복 저장하지 않습니다.
